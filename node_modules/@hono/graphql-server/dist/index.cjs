...
__export(index_exports, {
  ...
  getGraphQLParams: () => getGraphQLParams,
  graphqlServer: () => graphqlServer,
  respondWithGraphiQL: () => respondWithGraphiQL
});
...
var graphqlServer = (options) => {
  ...
  return async (c) => {
    ...
    const schemaValidationErrors = (0, import_graphql.validateSchema)(schema);
    ...
    let documentAST;
    try {
      documentAST = (0, import_graphql.parse)(new import_graphql.Source(query, "GraphQL request"));
    } catch (syntaxError) {
      if (syntaxError instanceof Error) {
        console.error(`${syntaxError.stack || syntaxError.message}`);
        const e = new import_graphql.GraphQLError(syntaxError.message, {
    ...
    const validationErrors = (0, import_graphql.validate)(schema, documentAST, [...import_graphql.specifiedRules, ...validationRules]);
    if (validationErrors.length > 0) {
      return c.json(errorMessages(["GraphQL validation error."], validationErrors), 400);
    }
    if (c.req.method === "GET") {
      const operationAST = (0, import_graphql.getOperationAST)(documentAST, operationName);
    ...
    try {
      result = await (0, import_graphql.execute)({
        schema,
      ...
      if (contextError instanceof Error) {
        ...
        const e = new import_graphql.GraphQLError(contextError.message, {
