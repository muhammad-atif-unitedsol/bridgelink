...
(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		module.exports = factory(require("react-native"), require("fastestsmallesttextencoderdecoder"));
	else if(typeof define === 'function' && define.amd)
		define(["react-native", "fastestsmallesttextencoderdecoder"], factory);
	else if(typeof exports === 'object')
		exports["Ably"] = factory(require("react-native"), require("fastestsmallesttextencoderdecoder"));
	else
		root["Ably"] = factory(root["react-native"], root["fastestsmallesttextencoderdecoder"]);
})(global, (__WEBPACK_EXTERNAL_MODULE__9925__, __WEBPACK_EXTERNAL_MODULE__8885__) => {
...
class Auth {
    ...
    async requestToken(tokenParams, authOptions) {
        ...
        const tokenRequest = (signedTokenParams, tokenCb) => {
            ...
            const requestHeaders = defaults_1.default.defaultPostHeaders(this.client.options, { format: Utils.Format.json });
...
class BaseClient {
    constructor(options) {
        ...
        this.MessageEncoding = basemessage_1.MessageEncoding;
...
class BaseRealtime extends baseclient_1.default {
    ...
    constructor(options) {
        ...
        this._liveObjectsPlugin = (_d = (_c = this.options.plugins) === null || _c === void 0 ? void 0 : _c.LiveObjects) !== null && _d !== void 0 ? _d : null;
...
DefaultRealtime._MessageEncoding = basemessage_1.MessageEncoding;
...
class PaginatedResult {
    ...
    async first() {
        ...
        throw new errorinfo_1.default('No link to the first page of results', 40400, 404);
    }
    async current() {
        if (this.hasCurrent()) {
            return this.get(this._relParams.current);
        }
        throw new errorinfo_1.default('No link to the current page of results', 40400, 404);
    }
    async next() {
        if (this.hasNext()) {
...
class Admin {
    ...
    async publish(recipient, payload) {
        ...
        const format = client.options.useBinaryProtocol ? Utils.Format.msgpack : Utils.Format.json, headers = defaults_1.default.defaultPostHeaders(client.options), params = {};
...
class DeviceRegistrations {
    ...
    async get(deviceIdOrDetails) {
        const client = this.client, format = client.options.useBinaryProtocol ? Utils.Format.msgpack : Utils.Format.json, headers = defaults_1.default.defaultGetHeaders(client.options), deviceId = deviceIdOrDetails.id || deviceIdOrDetails;
    ...
    async list(params) {
        const client = this.client, format = client.options.useBinaryProtocol ? Utils.Format.msgpack : Utils.Format.json, envelope = this.client.http.supportsLinkHeaders ? undefined : format, headers = defaults_1.default.defaultGetHeaders(client.options);
        Utils.mixin(headers, client.options.headers);
    ...
    async remove(deviceIdOrDetails) {
        const client = this.client, headers = defaults_1.default.defaultGetHeaders(client.options), params = {}, deviceId = deviceIdOrDetails.id || deviceIdOrDetails;
        if (typeof deviceId !== 'string' || !deviceId.length) {
...
class RealtimeAnnotations {
    ...
    async publish(msgOrSerial, annotationValues) {
        ...
        logger_1.default.logAction(this.logger, logger_1.default.LOG_MICRO, 'RealtimeAnnotations.publish()', 'channelName = ' +
            channelName +
    ...
    async subscribe(..._args /* [type], listener */) {
        ...
        if ((this.channel.state === 'attached' && this.channel._mode & protocolmessagecommon_1.flags.ANNOTATION_SUBSCRIBE) === 0) {
            throw new errorinfo_1.default("You are trying to add an annotation listener, but you haven't requested the annotation_subscribe channel mode in ChannelOptions, so this won't do anything (we only deliver annotations to clients who have explicitly requested them)", 93001, 400);
        }
    }
    ...
    async get(msgOrSerial, params) {
        return restannotations_1.default.prototype.get.call(this, msgOrSerial, params);
    }
}
...
class RealtimeChannel extends eventemitter_1.default {
    ...
    constructor(client, name, options) {
        ...
        if ((_c = client.options.plugins) === null || _c === void 0 ? void 0 : _c.LiveObjects) {
    ...
    async publish(...args) {
        ...
        if (size > maxMessageSize) {
            throw new errorinfo_1.default(`Maximum size of messages that can be published at once exceeded (was ${size} bytes; limit is ${maxMessageSize} bytes)`, 40009, 400);
        }
    ...
    sendState(objectMessages) {
        const msg = (0, protocolmessage_1.fromValues)({
            action: protocolmessagecommon_1.actions.OBJECT,
            channel: this.name,
    ...
    async processMessage(message) {
        if (message.action === protocolmessagecommon_1.actions.ATTACHED ||
            ...
            message.action === protocolmessagecommon_1.actions.OBJECT ||
            message.action === protocolmessagecommon_1.actions.ANNOTATION) {
            // RTL15b
        ...
        switch (message.action) {
            ...
            case protocolmessagecommon_1.actions.OBJECT:
            case protocolmessagecommon_1.actions.OBJECT_SYNC: {
                ...
                if (message.action === protocolmessagecommon_1.actions.OBJECT) {
    ...
    async getMessage(serialOrMessage) {
        logger_1.default.logAction(this.logger, logger_1.default.LOG_MICRO, 'RealtimeChannel.getMessage()', 'channel = ' + this.name);
        const restMixin = this.client.rest.channelMixin;
        return restMixin.getMessage(this, serialOrMessage);
    }
    async updateMessage(message, operation, params) {
        logger_1.default.logAction(this.logger, logger_1.default.LOG_MICRO, 'RealtimeChannel.updateMessage()', 'channel = ' + this.name);
        const restMixin = this.client.rest.channelMixin;
    ...
    async deleteMessage(message, operation, params) {
        logger_1.default.logAction(this.logger, logger_1.default.LOG_MICRO, 'RealtimeChannel.deleteMessage()', 'channel = ' + this.name);
        const restMixin = this.client.rest.channelMixin;
    ...
    async getMessageVersions(serialOrMessage, params) {
        logger_1.default.logAction(this.logger, logger_1.default.LOG_MICRO, 'RealtimeChannel.getMessageVersions()', 'channel = ' + this.name);
        const restMixin = this.client.rest.channelMixin;
...
class Rest {
    constructor(client) {
        ...
        this.PaginatedResource = paginatedresource_1.default;
        this.DeviceDetails = devicedetails_1.default;
        this.PushChannelSubscription = pushchannelsubscription_1.default;
        this.client = client;
    ...
    async stats(params) {
        ...
        return new paginatedresource_1.default(this.client, '/stats', headers, envelope, async (body, _, unpacked) => {
            const statsValues = unpacked
                ? body
                : Utils.decodeBody(body, this.client._MsgPack, format);
    ...
    async time(params) {
        const headers = defaults_1.default.defaultGetHeaders(this.client.options, { format: Utils.Format.json });
    ...
    async batchPublish(specOrSpecs) {
        ...
        const format = this.client.options.useBinaryProtocol ? Utils.Format.msgpack : Utils.Format.json, headers = defaults_1.default.defaultPostHeaders(this.client.options);
        if (this.client.options.headers)
            Utils.mixin(headers, this.client.options.headers);
    ...
    async batchPresence(channels) {
        const format = this.client.options.useBinaryProtocol ? Utils.Format.msgpack : Utils.Format.json, headers = defaults_1.default.defaultGetHeaders(this.client.options);
        if (this.client.options.headers)
            Utils.mixin(headers, this.client.options.headers);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.constructValidateAnnotation = exports.serialFromMsgOrSerial = void 0;
const tslib_1 = __webpack_require__(7582);
const Utils = tslib_1.__importStar(__webpack_require__(2678));
const annotation_1 = tslib_1.__importStar(__webpack_require__(9327));
const defaults_1 = tslib_1.__importDefault(__webpack_require__(3925));
...
function serialFromMsgOrSerial(msgOrSerial) {
    ...
    if (!messageSerial || typeof messageSerial !== 'string') {
        throw new errorinfo_1.default('First argument of annotations.publish() must be either a Message (or at least an object with a string `serial` property) or a message serial (string)', 40003, 400);
    }
    return messageSerial;
}
exports.serialFromMsgOrSerial = serialFromMsgOrSerial;
function constructValidateAnnotation(msgOrSerial, annotationValues) {
    ...
    if (!annotationValues || typeof annotationValues !== 'object') {
        throw new errorinfo_1.default('Second argument of annotations.publish() must be an object (the intended annotation to publish)', 40003, 400);
    }
...
class RestAnnotations {
    ...
    async publish(msgOrSerial, annotationValues) {
        ...
        const wireAnnotation = await annotation.encode();
        const client = this.channel.client, options = client.options, format = options.useBinaryProtocol ? Utils.Format.msgpack : Utils.Format.json, headers = defaults_1.default.defaultPostHeaders(client.options), params = {};
        Utils.mixin(headers, client.options.headers);
    ...
    async get(msgOrSerial, params) {
        const client = this.channel.client, messageSerial = serialFromMsgOrSerial(msgOrSerial), format = client.options.useBinaryProtocol ? Utils.Format.msgpack : Utils.Format.json, envelope = client.http.supportsLinkHeaders ? undefined : format, headers = defaults_1.default.defaultGetHeaders(client.options);
        Utils.mixin(headers, client.options.headers);
        return new paginatedresource_1.default(client, basePathForSerial(this.channel, messageSerial), headers, envelope, async (body, _, unpacked) => {
...
class RestChannel {
    ...
    async publish(...args) {
        ...
        const client = this.client, options = client.options, format = options.useBinaryProtocol ? Utils.Format.msgpack : Utils.Format.json, idempotentRestPublishing = client.options.idempotentRestPublishing, headers = defaults_1.default.defaultPostHeaders(client.options);
    ...
    async getMessage(serialOrMessage) {
        logger_1.default.logAction(this.logger, logger_1.default.LOG_MICRO, 'RestChannel.getMessage()', 'channel = ' + this.name);
        return this.client.rest.channelMixin.getMessage(this, serialOrMessage);
    }
    async updateMessage(message, operation, params) {
        logger_1.default.logAction(this.logger, logger_1.default.LOG_MICRO, 'RestChannel.updateMessage()', 'channel = ' + this.name);
        return this.client.rest.channelMixin.updateDeleteMessage(this, { isDelete: false }, message, operation, params);
    }
    async deleteMessage(message, operation, params) {
        logger_1.default.logAction(this.logger, logger_1.default.LOG_MICRO, 'RestChannel.deleteMessage()', 'channel = ' + this.name);
        return this.client.rest.channelMixin.updateDeleteMessage(this, { isDelete: true }, message, operation, params);
    }
    async getMessageVersions(serialOrMessage, params) {
        logger_1.default.logAction(this.logger, logger_1.default.LOG_MICRO, 'RestChannel.getMessageVersions()', 'channel = ' + this.name);
...
class RestChannelMixin {
    ...
    static history(channel, params) {
        const client = channel.client, format = client.options.useBinaryProtocol ? Utils.Format.msgpack : Utils.Format.json, envelope = channel.client.http.supportsLinkHeaders ? undefined : format, headers = defaults_1.default.defaultGetHeaders(client.options);
        Utils.mixin(headers, client.options.headers);
    ...
    static async status(channel) {
        ...
        const headers = defaults_1.default.defaultPostHeaders(channel.client.options);
    ...
    static async getMessage(channel, serialOrMessage) {
        ...
        if (!serial) {
            throw new errorinfo_1.default('This message lacks a serial. Make sure you have enabled "Message annotations, updates, and deletes" in channel settings on your dashboard.', 40003, 400);
        }
        const client = channel.client;
        const format = client.options.useBinaryProtocol ? Utils.Format.msgpack : Utils.Format.json;
        const headers = defaults_1.default.defaultGetHeaders(client.options);
        Utils.mixin(headers, client.options.headers);
        const { body, unpacked } = await resource_1.default.get(client, this.basePath(channel) + '/messages/' + encodeURIComponent(serial), headers, {}, null, true);
        const decoded = unpacked ? body : Utils.decodeBody(body, client._MsgPack, format);
        return (0, message_1._fromEncoded)(decoded, channel);
    }
    static async updateDeleteMessage(channel, opts, message, operation, params) {
        if (!message.serial) {
            throw new errorinfo_1.default('This message lacks a serial and cannot be updated. Make sure you have enabled "Message annotations, updates, and deletes" in channel settings on your dashboard.', 40003, 400);
        }
        const client = channel.client;
        const format = client.options.useBinaryProtocol ? Utils.Format.msgpack : Utils.Format.json;
        const headers = defaults_1.default.defaultPostHeaders(client.options);
        Utils.mixin(headers, client.options.headers);
        let encoded = null;
        if (message.data !== undefined) {
            encoded = await message_1.default.fromValues(message).encode(channel.channelOptions);
        }
        const req = {
        ...
        const requestBody = (0, message_1.serialize)(req, client._MsgPack, format);
        const method = opts.isDelete ? resource_1.default.post : resource_1.default.patch;
        const pathSuffix = opts.isDelete ? '/delete' : '';
        await method(client, this.basePath(channel) + '/messages/' + encodeURIComponent(message.serial) + pathSuffix, requestBody, headers, params || {}, null, true);
    }
    static getMessageVersions(channel, serialOrMessage, params) {
        ...
        const headers = defaults_1.default.defaultGetHeaders(client.options);
        Utils.mixin(headers, client.options.headers);
        return new paginatedresource_1.default(client, this.basePath(channel) + '/messages/' + encodeURIComponent(serial) + '/versions', headers, envelope, async (body, headers, unpacked) => {
...
class RestPresence {
    ...
    async get(params) {
        ...
        const client = this.channel.client, format = client.options.useBinaryProtocol ? Utils.Format.msgpack : Utils.Format.json, envelope = this.channel.client.http.supportsLinkHeaders ? undefined : format, headers = defaults_1.default.defaultGetHeaders(client.options);
        Utils.mixin(headers, client.options.headers);
...
class RestPresenceMixin {
    ...
    static async history(presence, params) {
        const client = presence.channel.client, format = client.options.useBinaryProtocol ? Utils.Format.msgpack : Utils.Format.json, envelope = presence.channel.client.http.supportsLinkHeaders ? undefined : format, headers = defaults_1.default.defaultGetHeaders(client.options);
        Utils.mixin(headers, client.options.headers);
        ...
        try {
            ...
            if (items && items.length)
                for (let i = 0; i < items.length; i++)
                    this.onProtocolMessage((0, protocolmessage_1.fromDeserialized)(items[i], this.connectionManager.realtime._RealtimePresence, this.connectionManager.realtime._Annotations, this.connectionManager.realtime._liveObjectsPlugin));
        }
        catch (e) {
...
class ConnectionManager extends eventemitter_1.default {
    constructor(realtime, options) {
        ...
        this.domains = defaults_1.default.getHosts(options);
        ...
        logger_1.default.logAction(this.logger, logger_1.default.LOG_MICRO, 'Realtime.ConnectionManager()', 'http domains = [' + this.domains + ']');
    ...
    getActiveTransportFormat() {
        var _a;
        return (_a = this.activeProtocol) === null || _a === void 0 ? void 0 : _a.getTransport().format;
    }
    /*********************
     * state management
     *********************/
    ...
    send(msg, queueEvent, callback) {
        ...
        if (this.logger.shouldLog(logger_1.default.LOG_MICRO)) {
            logger_1.default.logAction(this.logger, logger_1.default.LOG_MICRO, 'ConnectionManager.send()', 'queueing msg; ' +
                (0, protocolmessage_1.stringify)(msg, this.realtime._RealtimePresence, this.realtime._Annotations, this.realtime._liveObjectsPlugin));
        }
    ...
    constructor(message, callback) {
        ...
        this.ackRequired =
            typeof action === 'number' &&
                [protocolmessagecommon_1.actions.MESSAGE, protocolmessagecommon_1.actions.PRESENCE, protocolmessagecommon_1.actions.ANNOTATION, protocolmessagecommon_1.actions.OBJECT].includes(action);
    }
}
    ...
    send(pendingMessage) {
        ...
        if (this.logger.shouldLog(logger_1.default.LOG_MICRO)) {
            logger_1.default.logActionNoStrip(this.logger, logger_1.default.LOG_MICRO, 'Protocol.send()', 'sending msg; ' +
                (0, protocolmessage_1.stringify)(pendingMessage.message, this.transport.connectionManager.realtime._RealtimePresence, this.transport.connectionManager.realtime._Annotations, this.transport.connectionManager.realtime._liveObjectsPlugin));
        }
    ...
    onProtocolMessage(message) {
        if (this.logger.shouldLog(logger_1.default.LOG_MICRO)) {
            logger_1.default.logActionNoStrip(this.logger, logger_1.default.LOG_MICRO, 'Transport.onProtocolMessage()', 'received on ' +
                this.shortName +
                ': ' +
                (0, protocolmessage_1.stringify)(message, this.connectionManager.realtime._RealtimePresence, this.connectionManager.realtime._Annotations, this.connectionManager.realtime._liveObjectsPlugin) +
                '; connectionId = ' +
...
class WebSocketTransport extends transport_1.default {
    ...
    onWsData(data) {
        ...
        try {
            this.onProtocolMessage((0, protocolmessage_1.deserialize)(data, this.connectionManager.realtime._MsgPack, this.connectionManager.realtime._RealtimePresence, this.connectionManager.realtime._Annotations, this.connectionManager.realtime._liveObjectsPlugin, this.format));
        }
        catch (e) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.BaseMessage = exports.strMsg = exports.populateFieldsFromParent = exports.MessageEncoding = exports.encodeDataForWire = exports.wireToJSON = exports.decodeData = exports.decode = exports.encodeData = exports.encode = exports.encryptData = exports.normalizeCipherOptions = void 0;
const tslib_1 = __webpack_require__(7582);
const platform_1 = tslib_1.__importDefault(__webpack_require__(7400));
const Utils = tslib_1.__importStar(__webpack_require__(2678));
const errorinfo_1 = tslib_1.__importDefault(__webpack_require__(1798));
...
async function encryptData(data, encoding, cipherOptions) {
    ...
    if (!platform_1.default.BufferUtils.isBuffer(dataToEncrypt)) {
        dataToEncrypt = platform_1.default.BufferUtils.utf8Encode(String(dataToEncrypt));
    ...
    if (encoding) {
        ...
        try {
            while ((lastProcessedEncodingIndex = encodingsToProcess) > 0) {
                ...
                switch (xform) {
                    case 'base64':
                        decodedData = platform_1.default.BufferUtils.base64Decode(String(decodedData));
                        if (lastProcessedEncodingIndex == xforms.length) {
                            lastPayload = decodedData;
                        }
                        continue;
                    case 'utf-8':
                        decodedData = platform_1.default.BufferUtils.utf8Decode(decodedData);
                        continue;
                    case 'json':
                    ...
                    case 'vcdiff':
                        ...
                        try {
                            ...
                            decodedData = platform_1.default.BufferUtils.toBuffer(decodedData);
                            decodedData = platform_1.default.BufferUtils.arrayBufferViewToBuffer(context.plugins.vcdiff.decode(decodedData, deltaBaseBuffer));
                            lastPayload = decodedData;
                        }
                        catch (e) {
            ...
            const err = e;
            decodingError = new errorinfo_1.default(`Error processing the ${xform} encoding, decoder returned ‘${err.message}’`, err.code || 40013, 400);
        }
        finally {
            finalEncoding =
...
exports.encodeDataForWire = encodeDataForWire;
exports.MessageEncoding = {
    encryptData,
    encodeData,
    encodeDataForWire,
    decodeData,
...
const basemessage_1 = __webpack_require__(1976);
const Utils = tslib_1.__importStar(__webpack_require__(2678));
const actions = ['message.create', 'message.update', 'message.delete', 'meta', 'message.summary'];
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.stringify = exports.fromValues = exports.makeFromDeserializedWithDependencies = exports.fromDeserialized = exports.deserialize = exports.serialize = void 0;
const tslib_1 = __webpack_require__(7582);
const Utils = tslib_1.__importStar(__webpack_require__(2678));
const errorinfo_1 = tslib_1.__importDefault(__webpack_require__(1798));
const message_1 = __webpack_require__(3176);
...
function fromDeserialized(deserialized, presenceMessagePlugin, annotationsPlugin, objectsPlugin) {
    ...
    if (objectsPlugin && deserialized.state) {
        state = objectsPlugin.WireObjectMessage.fromValuesArray(deserialized.state, Utils, basemessage_1.MessageEncoding);
    }
    return Object.assign(new ProtocolMessage(), Object.assign(Object.assign({}, deserialized), { presence, messages, annotations, state, error }));
}
...
function makeFromDeserializedWithDependencies(dependencies) {
    return (deserialized) => {
        ...
        return fromDeserialized(deserialized, {
            PresenceMessage: presencemessage_1.default,
            WirePresenceMessage: presencemessage_1.WirePresenceMessage,
        }, { Annotation: annotation_1.default, WireAnnotation: annotation_1.WireAnnotation, RealtimeAnnotations: realtimeannotations_1.default, RestAnnotations: restannotations_1.default }, (_a = dependencies === null || dependencies === void 0 ? void 0 : dependencies.LiveObjectsPlugin) !== null && _a !== void 0 ? _a : null);
    };
}
exports.makeFromDeserializedWithDependencies = makeFromDeserializedWithDependencies;
function fromValues(values) {
...
function stringify(msg, presenceMessagePlugin, annotationsPlugin, objectsPlugin) {
    ...
    if (msg.state && objectsPlugin) {
        result +=
            '; state=' + toStringArray(objectsPlugin.WireObjectMessage.fromValuesArray(msg.state, Utils, basemessage_1.MessageEncoding));
    }
    if (msg.error)
...
exports.flags.MODE_ALL =
    exports.flags.PRESENCE |
        ...
        exports.flags.OBJECT_SUBSCRIBE |
        exports.flags.OBJECT_PUBLISH;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.getDefaults = exports.defaultPostHeaders = exports.defaultGetHeaders = exports.normaliseChannelOptions = exports.normaliseOptions = exports.objectifyOptions = exports.getAgentString = exports.getHosts = exports.getFallbackHosts = exports.endpointFallbacks = exports.getEndpointFallbackHosts = exports.getPrimaryDomainFromEndpoint = exports.getHttpScheme = exports.getPort = void 0;
const tslib_1 = __webpack_require__(7582);
const platform_1 = tslib_1.__importDefault(__webpack_require__(7400));
const Utils = tslib_1.__importStar(__webpack_require__(2678));
const logger_1 = tslib_1.__importDefault(__webpack_require__(1597));
...
exports.getPrimaryDomainFromEndpoint = getPrimaryDomainFromEndpoint;
/**
 * REC2c
 *
...
function checkIfClientOptionsAreValid(options) {
    ...
    if (options.endpoint && (options.environment || options.restHost || options.realtimeHost)) {
        // RSC1b
        throw new errorinfo_1.default('The `endpoint` option cannot be used in conjunction with the `environment`, `restHost`, or `realtimeHost` options.', 40106, 400);
    }
    // REC1c
    if (options.environment && (options.restHost || options.realtimeHost)) {
        // RSC1b
        throw new errorinfo_1.default('The `environment` option cannot be used in conjunction with the `restHost`, or `realtimeHost` options.', 40106, 400);
    }
}
function normaliseOptions(options, MsgPack, logger) {
    ...
    return Object.assign(Object.assign({}, options), { primaryDomain: primaryDomain, maxMessageSize: options.maxMessageSize || Defaults.maxMessageSize, timeouts,
...
function defaultGetHeaders(options, { format, protocolVersion = defaultHeadersOptions.protocolVersion } = {}) {
    const accept = contentTypes[format !== null && format !== void 0 ? format : (options.useBinaryProtocol ? Utils.Format.msgpack : Utils.Format.json)];
    return {
        accept: accept,
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.listenerToAsyncIterator = exports.withTimeoutAsync = exports.throwMissingPluginError = exports.createMissingPluginError = exports.arrEquals = exports.toBase64 = exports.matchDerivedChannel = exports.shallowEquals = exports.getGlobalObject = exports.getRetryTime = exports.getJitterCoefficient = exports.getBackoffCoefficient = exports.allToUpperCase = exports.allToLowerCase = exports.encodeBody = exports.decodeBody = exports.whenPromiseSettles = exports.arrChooseN = exports.randomString = exports.cheapRandStr = exports.dataSizeBytes = exports.inspectBody = exports.inspectError = exports.isErrorInfoOrPartialErrorInfo = exports.parseQueryString = exports.toQueryString = exports.arrPopRandomElement = exports.Format = exports.allSame = exports.forInOwnNonNullProperties = exports.valuesArray = exports.keysArray = exports.arrWithoutValue = exports.arrDeleteValue = exports.arrIntersectOb = exports.arrIntersect = exports.intersect = exports.containsValue = exports.inherits = exports.prototypicalClone = exports.shallowClone = exports.isNil = exports.isEmpty = exports.isObject = exports.ensureArray = exports.copy = exports.mixin = void 0;
const tslib_1 = __webpack_require__(7582);
const platform_1 = tslib_1.__importDefault(__webpack_require__(7400));
const errorinfo_1 = tslib_1.__importStar(__webpack_require__(1798));
...
function listenerToAsyncIterator(registerListener) {
    return tslib_1.__asyncGenerator(this, arguments, function* listenerToAsyncIterator_1() {
        const eventQueue = [];
        let resolveNext = null;
        const removeListener = registerListener((event) => {
            if (resolveNext) {
        ...
        try {
            while (true) {
                if (eventQueue.length > 0) {
                    ...
                    yield yield tslib_1.__await(eventQueue.shift());
                }
                else {
                    if (resolveNext) {
                        throw new errorinfo_1.default('Concurrent next() calls are not supported', 40000, 400);
                    }
                    // Otherwise wait for the next event to arrive
                    const event = yield tslib_1.__await(new Promise((resolve) => {
                        resolveNext = resolve;
                    }));
...
exports.listenerToAsyncIterator = listenerToAsyncIterator;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
function default_1(bufferUtils) {
    return {
        agent: 'reactnative',
        logTimestamps: true,
        ...
        atob: __webpack_require__.g.atob,
        nextTick: typeof __webpack_require__.g.queueMicrotask === 'function'
            ? (f) => __webpack_require__.g.queueMicrotask(f)
        ...
        getRandomArrayBuffer: (function (RNRandomBytes) {
            return async function (byteLength) {
                return new Promise((resolve, reject) => {
        ...
        })((__webpack_require__(9925).NativeModules).RNRandomBytes),
    };
}
...
class BufferUtils {
    ...
    sha256(message) {
        const hash = (0, hmac_sha256_1.sha256)(this.toBuffer(message));
 ...
 */
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.hmac = exports.sha256 = void 0;
...
module.exports = __WEBPACK_EXTERNAL_MODULE__8885__;
...
const config_1 = tslib_1.__importDefault(__webpack_require__(1676));
// @ts-ignore
const transport_1 = tslib_1.__importDefault(__webpack_require__(5655));
const logger_1 = tslib_1.__importDefault(__webpack_require__(1597));
const defaults_1 = __webpack_require__(3925);
const webstorage_1 = tslib_1.__importDefault(__webpack_require__(8949));
...
const Config = (0, config_1.default)(bufferutils_1.default);
const Crypto = (0, crypto_1.createCryptoClass)(Config, bufferutils_1.default);
...
platform_1.default.Config = Config;
...
exports["default"] = {
    ...
    makeProtocolMessageFromDeserialized: protocolmessage_1.makeFromDeserializedWithDependencies,
};
